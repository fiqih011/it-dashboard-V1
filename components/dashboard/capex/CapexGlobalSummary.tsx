...
interface CapexGlobalSummaryProps {
...
export default function CapexGlobalSummary({ data, loading }: CapexGlobalSummaryProps) {
  if (loading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <SummaryCard icon={<Briefcase className="w-5 h-5" />} title="Budget Plans" value="0" badge="entries" color="indigo" />
