...
import { X, RefreshCw, Calendar, User, Package, FileText } from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    "In Progres": "bg-blue-50 text-blue-700 border-blue-200",
...
export default function TransactionDetailModal({ open, budgetId, onClose }: Props) {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/dashboard/opex/transactions/${budgetId}`);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-gray-200">
        ...
        <div className="bg-gradient-to-r from-indigo-700 to-indigo-600 px-6 py-4 rounded-t-2xl flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-white">Detail Transaksi OPEX</h2>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-800 leading-snug">{budgetInfo.component}</p>
                    </div>
                  </div>
                </div>
