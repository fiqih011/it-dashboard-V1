...
export default function TransactionDetailModal({
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      console.log("📡 Fetching transactions for:", budgetId);
      
      const response = await fetch(
        `/api/dashboard/opex/transactions/${budgetId}` // ✅ URL match dengan [id]
      );
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, string> = {
      ...
      "In Progres": "bg-blue-50 text-blue-700 border-blue-300",
    };

    return (
      statusMap[status] || "bg-slate-50 text-slate-700 border-slate-300"
    );
  };
